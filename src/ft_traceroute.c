...
t_data g_data = {.options = {.family = AF_INET, .first_ttl = 1, .max_ttl = 30, .nprobes = 3, .datalen = 32, .resolve = true}};
...
int main(int argc, char **argv)
{
	...
	if (g_data.options.icmp)
	{
		...
		g_data.options.datalen += sizeof(*icmp);
		icmp->checksum = checksum((unsigned short *)buffer, g_data.options.datalen);
	}
	...
	for (unsigned int ttl = g_data.options.first_ttl; ttl <= g_data.options.max_ttl && !got_reply; ttl++)
	{
		...
		for (unsigned char probe = 0; probe < g_data.options.nprobes; probe++)
		{
			...
			{
				struct icmphdr *icmp = (struct icmphdr *)buffer;
				icmp->checksum -= htons(g_data.sequence) - icmp->un.echo.sequence;
